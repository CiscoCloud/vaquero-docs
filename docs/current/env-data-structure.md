...
* Inventory (NOTE: removed INI proposal from previous revision)
  * Each site has an inventory file that:
    1. Assigns hosts to a group (host -> host_group)
...
* Condensed metadata
  ...
  * A dedicated `host` collection will now exist when rendering any configuration templates. This will be populated with any information we have gained during the initial [iPXE chainloading](http://ipxe.org/cfg)
* JSON -> YAML
...
* Operating System definition: 
  * remove pw_hash/password from operating system
  * move cmdline from host_group -> operating system (for now)
...
A host matches a booting machine if _all_ selectors are present, and match. Selectors will be restricted to any information [pulled via iPXE](http://ipxe.org/cfg), or stored in our stateful information about this machine (assuming the machine has booted before and have collected facts about it).
